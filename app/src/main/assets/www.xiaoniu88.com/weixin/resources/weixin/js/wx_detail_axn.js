$(function(){
	...
	//请求数据URL
	var requestData = {
		tpl:"tpl/wx_detail_axn.tpl",
		data: "/weixin/productDetails/ptd/"+productId,
		record:"/weixin/invests/"+productId,
		//formSubmit:"/weChat/wx_axn_ainvest.do",
		formSubmit:"/product/trade/WeChat/buy"

		//data:"data/detail_axn.txt",
	...
	function submitAmount(pdata){
		...
		$.ajax({
			...
			beforeSend: function(){
				wx_BtnInvest.addClass("wx_btn_disabled").html("加入中...");
			},
			data:param,
			type:"post",
			dataType:"json",
			success: function(data){
				...
				if(data.status==0){
					...
					//更新成立日期
					$("#wxInvestDate").html($.formatTime(pdata.accrualTime,"yyyy-MM-dd"));
			...
			error:function(xhr,err){
				tip("系统繁忙，请稍后再试！");
				wx_BtnInvest.removeClass("wx_btn_disabled").html("立即加入");
			}
		});
	}
	//倒计时
	...
	function checkAmount(data){
		...
		if(leftMoney>=minTenderedSum){
			if(parseInt(val)<minTenderedSum){
				tip("单笔最小购买金额为"+minTenderedSum+"元");
				return false;
			}
		}
		//购买金额必须是1000的整数倍
		if (parseInt(val)> 1000 && parseInt(val)%1000!==0) {
			tip("购买金额需为1000元的整数倍!");
			return false;
		}
		//账户余额不足:购买金额大于账户余额
